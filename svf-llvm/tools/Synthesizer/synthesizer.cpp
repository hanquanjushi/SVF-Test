      ...
      //  if (const BranchStmt* branch = SVFUtil::dyn_cast<BranchStmt>(stmt))
       // {
            /*        std::string brstring = branch->getValue()->toString();
                   SVFVar* branchVar =
               const_cast<SVFVar*>(branch->getBranchInst()); SVFValue*
               branchValue = const_cast<SVFValue*>(branchVar->getValue());
                   ...
                   SVFVar* conditionVar =
               const_cast<SVFVar*>(branch->getCondition()); std::string
               conditionstring = conditionVar->getValue()->toString();
                   //   %5 = icmp slt i32 %4, 0, !dbg !17 { "ln": 7, "cl": 11,
               "fl":
                   //   "test1.c" }
                   std::size_t icmpPos = conditionstring.find("icmp");
                   if (icmpPos != std::string::npos)
                   {
                       std::size_t spacePos = conditionstring.find(" ",
               icmpPos); std::size_t nextSpacePos = conditionstring.find(" ",
               spacePos + 1); std::string operation = conditionstring.substr(
                       ...
                       lightAnalysis->findNodeOnTree(num, branch_order,
               operation, parameters, srcpath);
                   } */
      //  }
        ...
        else if (const CallPE* callPE = SVFUtil::dyn_cast<CallPE>(stmt))
        {
            ...
            /*            CallICFGNode* callNode =
                       ...
                       std::vector<std::string> parameters;
                       if (std::regex_search(callstring, match, re) &&
               match.size() > 2)
                       {
               ...
               ')); parameter.erase(parameter.find_last_not_of(' ') + 1);
                       ...
                       lightAnalysis->findNodeOnTree(num, call_order,
    ...
    /* for (const SVFFunction* F : svfModule->getFunctionSet())
    {
        ...
        if (m != "")
        {
               ...
               for (const SVFBasicBlock* bb : F->getBasicBlockList())
               {
                   for (const SVFInstruction* inst : bb->getInstructionList())
                   {
                       ...
                       if (flag == 1)
                       {
                           ...
                           std::string::size_type pos = m.find("\"ln\":");
                           unsigned int num =
                               std::stoi(m.substr(pos + 5, m.find(",") - pos -
           5)); std::regex re("@(\\w+)\\((.+)\\)"); std::smatch match;
                           std::string functionName;
                           pos = m.find("\"fl\": \"");
                           std::string srcpath =
           ...
           ')); parameter.erase(parameter.find_last_not_of(' ') + 1);
                           ...
                           std ::cout << srcpath << std::endl;
                           lightAnalysis->findNodeOnTree(num, call_order,
           functionName, parameters, srcpath);
                       }
                   }
               } 
    }*/
